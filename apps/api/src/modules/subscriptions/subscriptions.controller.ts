...
router.get('/usage', authenticate, async (req: Request, res: Response) => {
  ...
  const usagePercent = {
    ...
    encounters:
      limits.maxEncountersPerMonth !== Infinity
        ? (usage.encounterCount / limits.maxEncountersPerMonth) * 100
        : 0,
    ai:
      limits.maxAiRequestsPerMonth !== Infinity
        ? (usage.aiRequestCount / limits.maxAiRequestsPerMonth) * 100
        : 0,
...
router.post(
  ...
  async (req: Request, res: Response) => {
    ...
    if (existing)
      return res.status(409).json({ error: 'Subscription already exists for this clinic' });
...
router.post(
  ...
  async (req: Request, res: Response) => {
    ...
    if (!invoice)
      return res
        .status(400)
        .json({ error: 'Cannot generate invoice for free tier or missing subscription' });
