...
const ResolutionSchema = new Schema<IDisputeResolution>(
  {
    outcome: {
      type: String,
      enum: ['patient_favored', 'clinic_favored', 'no_action'],
      required: true,
    },
    notes: { type: String },
...
const PaymentDisputeSchema = new Schema<IPaymentDispute>(
  {
    ...
    reason: {
      type: String,
      enum: ['duplicate_payment', 'service_not_rendered', 'incorrect_amount', 'other'],
      required: true,
    },
    description: { type: String, required: true },
    status: {
...
export const PaymentDisputeModel = mongoose.model<IPaymentDispute>(
  'PaymentDispute',
