...
export async function up(db: Db): Promise<void> {
  ...
  await db
    ...
    .createIndex({ claimId: 1 }, { unique: true, background: true, name: 'claimId_1' });
  ...
  await db
    .collection('insuranceclaims')
    .createIndex({ status: 1, clinicId: 1 }, { background: true, name: 'status_1_clinicId_1' });
  ...
  await db
    .collection('insuranceclaims')
    .createIndex({ stellarTxHash: 1 }, { sparse: true, background: true, name: 'stellarTxHash_1' });
}
