...
async function fetchHighRiskPatients(): Promise<HighRiskPatient[]> {
  ...
  if (!res.ok) return [];
  const json = await res.json();
  return (json.data ?? []).filter((p: any) => p.riskLevel === 'high' || p.riskLevel === 'critical');
}
      ...
      {highRiskPatients.length > 0 && (
        <section aria-label="High-risk patients">
          <div className="border-danger-200 bg-danger-50 rounded-lg border p-4">
            <h2 className="text-danger-700 mb-3 flex items-center gap-2 text-sm font-semibold">
            ...
            <div className="space-y-2">
              {highRiskPatients.map((p) => (
                <Link
                  ...
                  className="hover:bg-danger-50 flex items-center justify-between rounded bg-white px-3 py-2 text-sm transition-colors"
                >
